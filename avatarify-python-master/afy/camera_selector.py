...
def query_cameras(n_cams):
    ...
    for camid in range(n_cams):
        ...
        log(f"Successfully read frame from camera with id {camid}")
        for i in range(10):
            ret, frame = cap.read()

        cam_frames[camid] = frame.copy()
        log(f"Captured and added frame from camera with id {camid}")

        cap.release()

    log(f"Finished querying cameras, found {len(cam_frames)} cameras")
    return cam_frames


def make_grid(images, cell_size=(320, 240), cols=2):
    log("Making grid of images")
    w0, h0 = cell_size
    ...
    for i, (camid, img) in enumerate(images.items()):
        ...
        log(f"Added camera {camid} to grid at position ({r}, {c})")
    return grid
...
def mouse_callback(event, x, y, flags, userdata):
    ...
    if event == 1:
        ...
        r = y // cell_size[1]
        camid = r * grid_cols + c
        log(f"Mouse click detected at ({x}, {y}) - selecting camera {camid}")
        if camid < len(cam_frames):
            g_selected_cam = camid
            log(f"Camera {camid} selected")


def select_camera(cam_frames, window="Camera selector"):
    log("Starting camera selection")
    cell_size = 320, 240
    grid_cols = 2
    ...
    if grid.shape[1] == 320:
        log("Only one camera available, resizing grid")
        cell_size = 640, 480
    ...
    while True:
        ...
        if g_selected_cam is not None:
            log(f"Camera {g_selected_cam} selected, breaking loop")
            break
        
        if key == 27:
            log("Escape key pressed, exiting camera selection")
            break

    cv2.destroyAllWindows()

    if g_selected_cam is not None:
        log(f"Selected camera {g_selected_cam}")
        return list(cam_frames)[g_selected_cam]
    else:
        log("No camera selected, returning default camera")
...
if __name__ == '__main__':
    ...
    log(f"Config loaded, querying {config['query_n_cams']} cameras")
    cam_frames = query_cameras(config['query_n_cams'])

    if cam_frames:
        selected_cam = select_camera(cam_frames)
        log(f"Selected camera {selected_cam}")
    else:
